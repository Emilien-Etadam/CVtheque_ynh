...
<body class="bg-gray-100 p-4" data-theme="dark">
    <div class="max-w-full mx-auto">
        ...
        <!-- Header séparé pour les filtres et l'envoi de PDF -->
        <div class="bg-white p-4 rounded-lg shadow-md mb-4">
            <div class="flex flex-col md:flex-row gap-4">
                <!-- Filtres (colonne gauche) -->
                <div class="w-full md:w-1/2 bg-gray-50 p-3 rounded-lg">
                    <h3 class="text-lg font-medium mb-2">Filtres</h3>
                    <form action="" method="get" class="space-y-3">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Statut</label>
                            <select name="filter_status" class="w-full p-2 border rounded" onchange="this.form.submit()">
                        ...
                        <div>
                            ...
                            <select name="filter_position" class="w-full p-2 border rounded" onchange="this.form.submit()">
                ...
                <!-- Ajouter un candidat (colonne droite) -->
                <div class="w-full md:w-1/2 bg-white p-3 rounded-lg shadow-md">
                    <h3 class="text-lg font-medium mb-2 text-center">Ajouter un candidat</h3>
        ...
        <div class="flex justify-between items-center mb-4">
            ...
            <div class="flex items-center">
                <!-- Tous les contrôles dans un seul conteneur avec le même style -->
                <div class="view-toggle">
                    <div class="view-toggle-buttons">
                        <!-- Contrôles de zoom (visibles uniquement en vue tuile) -->
                        <div id="zoomControls" class="hidden" style="display: flex;">
                            <button type="button" id="zoomOutBtn" class="view-toggle-button" aria-label="Réduire">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4" />
                                </svg>
                            </button>
                            <button type="button" id="zoomInBtn" class="view-toggle-button" aria-label="Agrandir">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <button type="button" id="tableViewBtn" class="view-toggle-button active" aria-label="Vue tableau">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M3 14h18m-9-4v8m-7 0h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
                            </svg>
                        </button>
                        <button type="button" id="tileViewBtn" class="view-toggle-button" aria-label="Vue tuile">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <!-- Conteneur principal avec les vues -->
        <div class="flex flex-col md:flex-row gap-2" id="mainContainer">
            <!-- Liste des candidats (colonne gauche) -->
            <div class="w-full md:w-1/2 bg-white p-3 rounded-lg shadow-md" id="candidatesContainer">
                ...
                <!-- Vue en tuile (Pinterest-like) -->
                <div id="tileView" class="tile-container hidden">
                    <?php 
                    // Réinitialiser le curseur de résultat pour réafficher les candidats
                    if ($candidates) {
                        $candidates->reset();
                        while ($candidate = $candidates->fetchArray(SQLITE3_ASSOC)): 
                            // Déterminer la classe CSS pour le statut
                            $statusClass = 'status-' . str_replace(' ', '-', $candidate['status']);
                    ?>
                    <div class="candidate-tile" data-id="<?= $candidate['id'] ?>" data-cv-path="<?= htmlspecialchars($candidate['cv_path']) ?>">
                        <!-- Aperçu du PDF avec tous les overlays à l'intérieur -->
                        <div class="tile-pdf-preview">
                            ...
                            <!-- Positionnement du statut en bas à gauche de l'image -->
                            <div class="tile-status-overlay">
                                <div class="tile-status <?= $statusClass ?>"><?= htmlspecialchars($candidate['status']) ?></div>
                            </div>
                            
                            <!-- Positionnement des icônes d'action en bas à droite de l'image -->
                            <div class="tile-actions-overlay">
                                <a href="javascript:void(0)" onclick="toggleEdit(<?= $candidate['id'] ?>)" 
                                   class="tile-action edit-toggle" title="Éditer">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </a>
                                <a href="?delete=<?= $candidate['id'] ?>" class="tile-action delete" 
                                   onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce candidat ?')" title="Supprimer">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                    </svg>
                            ...
                            <div class="tile-comments-hover">
                                <?= nl2br(htmlspecialchars($candidate['comments'])) ?>
                            </div>
                            <?php endif; ?>
                        </div>
                        
                        <div class="tile-body">
                            <!-- Nom et poste sur une colonne -->
                            <div class="tile-name"><?= htmlspecialchars($candidate['name']) ?></div>
                            <div class="tile-position"><?= htmlspecialchars($candidate['position']) ?></div>
                            
                            <!-- Email cliquable et date (sans les mots "Email" et "Date") -->
                            <div class="tile-info">
                                <a href="mailto:<?= htmlspecialchars($candidate['email']) ?>" class="tile-email"><?= htmlspecialchars($candidate['email']) ?></a>
                                <span class="tile-date"><?= htmlspecialchars($candidate['application_date']) ?></span>
                            </div>
                        </div>
                    </div>
                    <?php endwhile; } ?>
